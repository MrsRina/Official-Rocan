...
public class ModuleOffhand extends Module {
    ...
    public static ValueNumber settingSmartTotem = new ValueNumber("Smart Totem", "SmartTotem", "If 0 its disabled, make you don't die.", 1, 0, 36);
    ...
    public void onListen(ClientTickEvent event) {
        ...
        if (settingAutoTotem.getValue() && (settingTotem.getState() && settingEndCrystal.getState() && settingGoldenApple.getState() && settingBow.getState()) == false) {
    ...
    public void doOffhand(Item item) {
        ...
        if (item != Items.TOTEM_OF_UNDYING && settingSmartTotem.getValue().intValue() != 0 && mc.player.getHealth() <= settingSmartTotem.getValue().intValue()) {
